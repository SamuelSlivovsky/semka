module.exports = {
  ...
  getVolnyPredajZdrPomocok: (req, res) => {
    const volnyPredajZdrPomocok = require("../models/lekarensky_sklad");
    (async () => {
      result = await volnyPredajZdrPomocok.getVolnyPredajZdrPomocok(
        req.params.id
      );
      res.status(200).json(result);
    })();
  ...
  updatePocetVolnopredajnejZdrPomocky: (req, res) => {
    const volnopredajnaZdrPomocka = require("../models/lekarensky_sklad");
    (async () => {
      ret_val =
        await volnopredajnaZdrPomocka.updatePocetVolnopredajnejZdrPomocky(
          req.body
        );
      res.status(200).json("success");
    })().catch((err) => {

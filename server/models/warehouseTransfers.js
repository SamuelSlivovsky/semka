...
async function getHospitalMedication(id) {
    try {
        ...
        const sqlStatement = `SELECT LIEK.ID_LIEK, LIEK.NAZOV, TO_CHAR(MIN(unique(TRVANLIVOST_LIEKU.DATUM_TRVANLIVOSTI)),'DD.MM.YYYY') AS DATUM_TRVANLIVOSTI,
           SKLAD.ID_ODDELENIA AS ID_ODDELENIA, TYP_ODDELENIA, (SKLAD.CELKOVY_POCET - SKLAD.MINIMALNY_POCET) AS POCET FROM SKLAD
            ...
            GROUP BY LIEK.ID_LIEK, LIEK.NAZOV, SKLAD.CELKOVY_POCET, SKLAD.MINIMALNY_POCET, SKLAD.ID_ODDELENIA, TYP_ODDELENIA`;
        let result = await conn.execute(sqlStatement, {
            search_id: id
        });
        return result.rows;

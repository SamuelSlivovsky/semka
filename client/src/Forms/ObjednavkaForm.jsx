...
export default function ExaminationForm(props) {
    ...
    const [filteredLekari, setFilteredLekari] = useState(null);

    useEffect(() => {
        const token = localStorage.getItem('user');
        const headers = { authorization: 'Bearer ' + token };
        fetch(`/add/lekariPacienta/${GetUserData(token).UserInfo.userid}`, { headers })
    ...
    const searchLekari = (event) => {
        setTimeout(() => {
            let _filteredPsc;
            if (!event.query.trim().length) {
                _filteredPsc = [...lekari];
            } else {
                _filteredPsc = lekari.filter((city) => {
                    return city.name.toLowerCase().startsWith(event.query.toLowerCase());
                });
            }

            setFilteredLekari(_filteredPsc);
        }, 250);
    };
                                ...
                                render={({ input, meta }) => (
                                    <div className='field col-12'>
                                        <span className='p-float-label'>
                                            <AutoComplete
                                                {...input}
                                                suggestions={filteredLekari}
                                                completeMethod={searchLekari}
                                                field='name'
                                                className={classNames({
                                                    'p-invalid': isFormFieldValid(meta),
                                                })}

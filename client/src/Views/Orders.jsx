...
export default function Orders() {
      ...
      .then((data) => {
        try {
          ...
          if (jsonData && jsonData[0] && jsonData[0].ZOZNAM_LIEKOV) {
            ...
            const xmlContentArray = Array.isArray(zoznamLiekovArray)
              ? zoznamLiekovArray
              : [zoznamLiekovArray];
            setLoading(false);
    ...
    if (xmlContent.length === 0) {
      ...
      if (finish) {
        ...
        const _products = products.filter(
          (product) => product.ID_OBJEDNAVKY !== order.ID_OBJEDNAVKY
        );
  ...
  const handleInputChange = (
    ...
    let inputValue =
      value === ""
        ? ""
        : Math.min(parseInt(value.replace(/\D/g, ""), 10), item.amount);
